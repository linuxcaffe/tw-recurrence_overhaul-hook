#!/usr/bin/env python3
"""
Taskwarrior Enhanced Recurrence Hook - On-Modify (DEBUG VERSION)
Based on the Recurrence Overhaul RFC

Installation:
1. Save to ~/.task/hooks/on-modify_recurrence.py
2. chmod +x ~/.task/hooks/on-modify_recurrence.py
"""
...
def debug_log(msg):
    ...
    if DEBUG:
        with open(LOG_FILE, 'a') as f:
            ...
            f.write(f"[{timestamp}] MODIFY: {msg}\n")

debug_log("="*60)
debug_log("on-modify hook started")
...
class RecurrenceHandler:
    ...
    def create_template(self, task):
        ...
        if not anchor_field or not anchor_date:
            ...
            print("ERROR: Recurring task must have either 'due' or 'scheduled' date", 
                  file=sys.stderr)
            sys.exit(1)
    ...
    def handle_template_modification(self, original, modified):
        ...
        if modified.get('status') in ['completed', 'deleted']:
            ...
            print("ERROR: Cannot complete or delete a recurrence template.", file=sys.stderr)
            print("Delete the instances first, or modify the template instead.", file=sys.stderr)
            sys.exit(1)
...
def main():
    """Main hook entry point for on-modify"""
    ...
    if len(lines) < 2:
        debug_log("Less than 2 lines, returning first line if exists")
        if lines:
    ...
    if 'r' in modified and modified.get('status') != 'recurring':
        debug_log("Creating new template")
        modified = handler.create_template(modified)
        print(json.dumps(modified))
        print("Created recurrence template. First instance will be generated on exit.")
        debug_log("Template creation complete, exiting")
        sys.exit(0)
