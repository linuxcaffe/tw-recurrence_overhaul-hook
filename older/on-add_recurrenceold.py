...
Changelog:
  0.3.1 (2026-01-10):
    - No changes (version sync with on-exit)
    
  0.3.0 (2026-01-09):
    - BREAKING: Removed uda.type.values restriction (hook validates now)
    - Added: Block 'until' on chained tasks (prevents infinite loop)
    - Added: rlimit UDA for controlled instance pile-up (periodic only)
    - Added: 'recur' field copied to instances for native R indicator
    - Improved: Type abbreviation accepts any unambiguous prefix (c, ch, p, pe, etc)
    - Improved: Default type is 'periodic' if not specified
    - Improved: Input validation with descriptive error messages
    - Fixed: Type normalization happens before any validation
    
  0.2.0 (2026-01-08):
    - Fixed: Template deletion bug (check original status first)
    - Fixed: Type abbreviation support (normalize before Taskwarrior validates)
    - Added: Allow template deletion instead of blocking
    - Added: Pre-normalization of type field in both on-add and on-modify
...
class RecurrenceHandler:
    ...
    def normalize_type(self, type_str):
        """
        Normalize type abbreviations to full names with validation.
        Accepts any unambiguous prefix: c, ch, chai, p, pe, per, etc.
        """
        if not type_str:
        ...
        # Chained variants
        if type_lower in ['c', 'ch', 'cha', 'chai', 'chain', 'chaine', 'chained']:
            return 'chained'
        
        # Periodic variants
        if type_lower in ['p', 'pe', 'per', 'peri', 'perio', 'period', 'periodi', 'periodic']:
            return 'periodic'
        
        # Invalid type - error with helpful message
        sys.stderr.write(f"ERROR: Invalid recurrence type '{type_str}'.\n")
        sys.stderr.write("       Must be 'chained' (c/ch/chai...) or 'periodic' (p/pe/per...).\n")
        sys.exit(1)
    ...
    def create_template(self, task):
        ...
        # Validate: chained tasks cannot use 'until' dates
        if task['type'] == 'chained' and 'until' in task:
            sys.stderr.write("ERROR: Chained recurrence cannot use 'until' dates.\n")
            sys.stderr.write("       Chained tasks spawn based on completion, not time.\n")
            sys.stderr.write("       Use 'rend' to limit recurrence, or delete the template to stop.\n")
            sys.exit(1)
        
        # Set default rlimit if not specified (for periodic pile-up control)
        if 'rlimit' not in task:
            task['rlimit'] = '1'  # Default: one instance at a time
        
        if DEBUG:
            debug_log(f"  Type: {task['type']}, r={task.get('r')}, rlimit={task.get('rlimit')}")
    ...
    def handle_template_modification(self, original, modified):
        ...
        if modified.get('status') in ['completed', 'deleted']:
            # Allow deletion/completion - just pass through
            if DEBUG:
                debug_log(f"Template being {modified.get('status')}: {original.get('description')}")
            return modified
...
def main():
    ...
    if IS_ON_ADD:
        ...
        # Pre-normalize type if present (before create_template)
        if 'type' in task:
            task['type'] = handler.normalize_type(task['type'])
        ...
        # Pre-normalize type if present (before any processing)
        if 'type' in modified:
            modified['type'] = handler.normalize_type(modified['type'])
        
        # Modifying a template? (check this FIRST)
        if handler.is_template(original):
            modified = handler.handle_template_modification(original, modified)
        
        # Adding recurrence to existing task?
        elif 'r' in modified and modified.get('status') != 'recurring':
